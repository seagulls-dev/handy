<div class="form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
        <textarea id="{{$field}}" class="form-control col-md-7 col-xs-12 @if($errors->has($field)) parsley-error @endif"
               name="{{$field}}" @if(!empty($required)) required @endif
                  @if(!empty($readonly)) readonly @endif>{{ old($field) ?: (isset($value) ? $value : '')  }}</textarea>
        @if($errors->has($field))
